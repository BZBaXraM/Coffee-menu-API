...
**Size variants**: Dishes can have optional size variants (e.g. milkshakes S/M) stored in the `dishes.sizes` column as a JSON array of `{ label, price }` (price in AZN). Empty `[]` means no sizes — the plain `dish.price` applies. The `price` column mirrors the smallest size and is used on cards / by the AI as the "from" price and as a fallback. The seed gives the four milkshakes S (4₼) / M (5₼); `db/database.js` also idempotently backfills these onto existing DBs (by English name, only when `sizes` is empty), alongside an `ALTER TABLE` migration that adds the column. Frontend helper `dishSizes(dish)` (in `api.js`) parses the column safely. `DishModal` renders a size selector and adds the chosen variant; `DishCard`'s quick-add opens the modal (instead of adding directly) when a dish has sizes; `AIChat` quick-add defaults to the smallest size. In the cart, each size is a **distinct line** keyed by `${id}::${label}` (see `CartContext` `lineKey` — `add(dish, qty, size)` takes an optional `{label, price}`; `remove`/`updateQty` are keyed by `item.key`, with a backfill for pre-sizes carts). The size label is shown in `CartDrawer`, included in the WhatsApp message, and saved on each order item. Admins manage sizes in the dish form (`AdminPage` `DishForm`), which normalizes rows (numeric prices, no blank labels) before saving.
